...
&НаСервере
Процедура тгКнопкиНаСервере()
	Сервер = "api.telegram.org";
	КлючАпи = "bot6131297068:AAHk1AbCRpq5gm4kkYiKKp__vfREeDOZASY/";
	тгЗапрос = "setChatMenuButton";  
	...
	ТелоЗапроса.Вставить("chat_id", ЧатИД);
	ТелоЗапроса.Вставить("text", "врврвиы");

	ДЖСОН = Новый ЗаписьJSON;
	ДЖСОН.УстановитьСтроку();
	ЗаписатьJSON(ДЖСОН, ТелоЗапроса);
	
	Запрос = Новый HTTPЗапрос();  
	Запрос.Заголовки = Заголовки;
	Запрос.АдресРесурса = КлючАпи + тгЗапрос;
	Запрос.УстановитьТелоИзСтроки(ДЖСОН.Закрыть());
	
	Ответ = Соединение.ВызватьHTTPМетод("POST", Запрос);
	
	Если ПроверкаОтвета(Ответ) Тогда
...
Функция ПроверкаОтвета(Ответ)  
	...
	Если Ответ.КодСостояния >= 400 и Ответ.КодСостояния < 500 Тогда
		Сообщить("Ошибка, запрос не прошел! Код ошибки:" + Ответ.КодСостояния);
		Возврат Ложь;
	КонецЕсли;	
	
	Если Ответ.КодСостояния >= 500 Тогда
		Сообщить("Ошибка,на стороне сервера! Код ошибки:" + Ответ.КодСостояния);	   
		Возврат Ложь;
	КонецЕсли;
...
&НаСервере
Процедура ОтобразитьОтвет(Ответ)

	ТелоОтвета = Ответ.ПолучитьТелоКакСтроку();
	
	
	ПараметрыЗаписиДЖСОН = Новый ПараметрыЗаписиJSON(
		ПереносСтрокJSON.Авто,
		Символы.Таб);
	
	
	ЧтениеДЖСОН = Новый ЧтениеJSON;
	ЧтениеДЖСОН.УстановитьСтроку(ТелоОтвета);
	
	СтруктураИзДЖСОН = ПрочитатьJSON(ЧтениеДЖСОН);
	
	ЧтениеДЖСОН.Закрыть();

	ЗаписьДЖСОН = Новый ЗаписьJSON; 
	ЗаписьДЖСОН.УстановитьСтроку(ПараметрыЗаписиДЖСОН);
	ЗаписатьJSON(ЗаписьДЖСОН, СтруктураИзДЖСОН);  
	
	ОтветСервера = ЗаписьДЖСОН.Закрыть();	
КонецПроцедуры 
